<?php
	...
	$pdf->SetTitle( "Les Chroniques d'Akéras - Personnages vivants" );
	$pdf->SetSubject( "Impression de tous les personnages vivants" );
	...
	foreach( $chars as $character_infos ){
		$prestige_voie = 0;
		$prestige_desc = "";
		if( $character_infos->prestige_id != 0 ){
			foreach( $list_prestiges as $prestige_id => $prestige_infos ){
				if( $character_infos->prestige_id == $prestige_id ){
					$prestige_voie = $prestige_infos[ "voie_id" ];
					$prestige_desc = $prestige_infos[ "nom" ];
				}
			}
		}
		...
		foreach( $character_infos->sorts as $sphere_id => $sorts_sphere ){
			...
			foreach( $sorts_sphere as $sort_id ){
				...
				if( array_key_exists( $sort_id, $list_sorts_cercles ) &&
						isset( $list_sorts_cercles[ $sort_id ][ "nom" ] ) &&
						isset( $list_sorts_cercles[ $sort_id ][ "cercle" ] )
				){
					$tmp_sorts[ $sort_id ] = $list_sorts_cercles[ $sort_id ][ "nom" ] . " (" . $sphere_desc . " " . $list_sorts_cercles[ $sort_id ][ "cercle" ] . ")";
				} else {
		...
		$pdf->AddPage();
		$pdf->SetXY( PDF_MARGIN_LEFT, PDF_MARGIN_TOP );
		...
		$pdf->Rect( $pdf->GetX(), $pdf->GetY(),
				$width_third, ( $pdf->GetCellHeight( $pdf->GetFontSize() ) * 8.5 ) );
		...
		$pdf->Cell( 25, 0, "F. Arcanique : " );
		$pdf->Cell( $width_third - $width_buffer - 25, 0, $character_infos->GetForceArcanique(), "B", 0, "C" );
		$pdf->Cell( $width_buffer * 2, 0, "" );
		...
		$pdf->Cell( 25, 0, "F. Divine : " );
		$pdf->Cell( $width_third - $width_buffer - 25, 0, $character_infos->GetForceDivine(), "B", 0, "C" );
		$pdf->Cell( $width_buffer * 2, 0, "" );
		$pdf->Cell( 25, 0, "Sagesse" );
		AddSelections( $pdf, $character_infos->sagesse );
		$pdf->Cell( $width_third + $width_buffer - 25, 0, "");
	...
	$pdf->Output( "CdA_ALL_" . time() . ".pdf", "I" );
